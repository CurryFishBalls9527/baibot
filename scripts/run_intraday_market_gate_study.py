#!/usr/bin/env python3
"""Does a composite market-context gate salvage gap_reclaim's 2020 perf?

Open question 2 from `memory/project_intraday_baseline_study.md`. The
post-fix gap_only_filter_on baseline shows fair-weather behavior:
+12.0% IS / -13.9% 2020 / -3.7% 2018. The single-symbol SMA20
daily_trend_filter doesn't help in 2020 because individual stocks
chase up briefly inside the broader crash.

Hypothesis: requiring the composite SPY/QQQ/IWM/SMH/^VIX score to be
>= some threshold gates out crisis days entirely. If 2020 R/DD flips
positive without crushing trade count, that's a real regime fix. If
it just drops 2020 trades to noise (<30), it's just survivor cherry-pick.

Variants:
- baseline             — existing daily_trend_filter only (post-fix)
- mctx_score_4         — also require composite_score >= 4 (drop correction)
- mctx_score_5         — also require composite_score >= 5 (uptrend pressure+)
- mctx_score_6         — also require composite_score >= 6 (confirmed_uptrend)

Backtester: post-fix (`<` boundary) with the new market_context_min_score
...
# Same as baseline study (gap_only_filter_on), 15m, broad250
...
VARIANTS = {
    "baseline":     {"market_context_min_score": None},
    "mctx_score_4": {"market_context_min_score": 4},
    "mctx_score_5": {"market_context_min_score": 5},
    "mctx_score_6": {"market_context_min_score": 6},
}
...
def main() -> int:
    ...
    out_dir = Path("results/intraday_market_gate_study")
    out_dir.mkdir(parents=True, exist_ok=True)
    print(f"Universe: {UNIVERSE_PATH} ({len(symbols)} syms)")
    print("Post-fix backtester + market_context gate (`<` boundary)\n")
    ...
    print()
    print(f"{'variant':14s}{'period':12s}{'ret%':>9s}{'DD%':>7s}{'R/DD':>7s}{'trades':>8s}{'WR%':>6s}")
    print("-" * 63)
    for v, p, ret, dd, rdd, tr, wr in rows:
        print(f"{v:14s}{p:12s}{ret:+9.2f}{dd:7.2f}{rdd:7.2f}{tr:8d}{wr:6.1f}")

    print()
    print("R/DD pivot (signed: + means positive return):")
    print(f"  {'variant':14s}{'2023_2025':>12s}{'2020':>10s}{'2018':>10s}")
    for v in VARIANTS:
        cells = []
