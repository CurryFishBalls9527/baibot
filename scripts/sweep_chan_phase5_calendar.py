#!/usr/bin/env python3
"""Phase 5 — Calendar / seasonality gate sweep."""
from __future__ import annotations
import json, logging, subprocess, sys
from pathlib import Path

logging.basicConfig(level=logging.INFO, format="%(asctime)s %(message)s", datefmt="%H:%M:%S")
log = logging.getLogger("phase5_cal")
...
def main():
    out_base = "results/chan_daily_etf/phase5_calendar"
    ...
    cells = {}
    cells["sell_in_may"] = run_cell("sell_in_may", ["--calendar-filter", "sell_in_may"], out_base)
    cells["santa_only"] = run_cell("santa_only", ["--calendar-filter", "santa_only"], out_base)
    # Block specific weak months
    for ms_name, ms in [("block_sep", "9"), ("block_aug_sep", "8,9"), ("block_may_sep", "5,6,7,8,9")]:
        cells[ms_name] = run_cell(ms_name, ["--calendar-filter", "block_months", "--calendar-block-months", ms], out_base)

    fmt("CALENDAR GATES (vs baseline)", cells, baseline_avg=bavg)
