#!/usr/bin/env python3
"""Phase 5 — Portfolio-level vol-target sweep."""
from __future__ import annotations
import json, logging, subprocess, sys
from pathlib import Path

logging.basicConfig(level=logging.INFO, format="%(asctime)s %(message)s", datefmt="%H:%M:%S")
log = logging.getLogger("phase5_voltarget")
...
def main():
    out_base = "results/chan_daily_etf/phase5_voltarget"
    ...
    for tgt in [0.10, 0.15, 0.20]:
        for lb in [20, 30, 60]:
            label = f"voltgt{tgt}_lb{lb}"
            cells[label] = run_cell(label,
                ["--portfolio-vol-target", str(tgt),
                 "--portfolio-vol-lookback", str(lb)],
                out_base)
    fmt("PORTFOLIO VOL TARGET (vs baseline)", cells, baseline_avg=bavg)
