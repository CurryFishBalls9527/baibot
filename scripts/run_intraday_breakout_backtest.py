#!/usr/bin/env python3
"""Run the research-only mechanical intraday breakout prototype.

This is an additive validation harness for intraday data + backtest plumbing.
It does not alter any existing live-paper strategy or scheduler path.
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="Run intraday breakout prototype backtest")
    ...
    p.add_argument("--out", default="results/intraday_breakout/backtest.json")
    return p.parse_args()
...
def main():
    ...
    if args.apply_tradability_filter:
        symbols, diagnostics = filter_symbols_by_tradability(
            symbols,
            as_of_date=args.end,
    ...
    result = bt.backtest_portfolio(symbols, args.begin, args.end, args.db)

    summary = result.summary
    print()
    print("=" * 70)
    print("  INTRADAY MECHANICAL BREAKOUT — PROTOTYPE")
    print("=" * 70)
