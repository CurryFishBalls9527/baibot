#!/usr/bin/env python3
"""Phase 5 — Re-entry after stop sweep + lag probes for block_aug_sep winner."""
from __future__ import annotations
import json, logging, subprocess, sys
from pathlib import Path

logging.basicConfig(level=logging.INFO, format="%(asctime)s %(message)s", datefmt="%H:%M:%S")
log = logging.getLogger("phase5_reentry")
...
def run_cell(cell_name, override_flags, out_base, override_lag=None):
    flags = list(OPTIMAL_FLAGS)
    if override_lag is not None:
        # Replace existing entry-lag-extra-days
        for i in range(len(flags)):
            if flags[i] == "--entry-lag-extra-days":
                flags[i + 1] = str(override_lag)
                break
...
def main():
    out_base = "results/chan_daily_etf/phase5_reentry"
    ...
    for w in [3, 5, 10]:
        for c in [1, 2]:
            label = f"reentry_w{w}_c{c}"
            cells[label] = run_cell(label,
                ["--reentry-after-stop",
                 "--reentry-window-bars", str(w),
                 "--reentry-max-count", str(c)],
                out_base)
    fmt("RE-ENTRY AFTER STOP (vs baseline)", cells, baseline_avg=bavg)
